...
function library:AddWindow(title, options)
    ...
    function Dragger.new(frame)
        ...
        self.startInputPosition = nil
        self.draggable = true
        self.draggerCache = true

        self.frame.Active = true

        self.frame.MouseLeave:Connect(
            function()
                self.draggerCache = false
            end
        )

        self.frame.MouseEnter:Connect(
            function()
                self.draggerCache = true
            end
        )
        ...
        function self:onInputChanged(input)
            if self.held then
                ...
                local delta = currentMousePos - self.startInputPosition
                local newPosition = self.startPosition + UDim2.new(0, delta.X, 0, delta.Y)
                self.frame.Position = self.frame.Position:Lerp(newPosition, 0.05)
            end
        end

        function self:onInputEnded(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                self.held = false
            end
        end

        self.frame.InputBegan:Connect(
            function(input)
                if
                    input.UserInputType == Enum.UserInputType.MouseButton1 or
                        input.UserInputType == Enum.UserInputType.Touch
                 then
                    if self.draggerCache and self.draggable then
                        self.held = true
                        self.startPosition = self.frame.Position
                        self.startInputPosition = input.Position
    ...
    function Dragger:setDraggable(isDraggable)
        self.draggable = isDraggable
    end

    local myDragger = Dragger.new(Window)

	do -- Resize Window
		local oldIcon = mouse.Icon
		local Entered = false
		Resizer.MouseEnter:Connect(function()
			myDragger:setDraggable(false)
			...
			function window_data:AddTab(tab_name)
				...
				do -- Tab Elements
					...
					function tab_data:AddKeybind(keybind_name, callback, keybind_options)
                        ...
                        local scrollPos = Vector2.new(0, 0)
                        function tab_data:AddDropdown(dropdown_name, callback)
                            ...
                            dropdown.Activated:Connect(function()
                                ...
                                if not open then
                                    scrollPos = objects.CanvasPosition
                                end
                                ...
                                if open then -- Open
                                    if dropdown_open then return end
                                    dropdown_open = true
                                    local boxTween = Resize(box, {Size = UDim2.new(1, 0, 0, len)}, options.tween_time)
                                    local indicatorTween = Resize(indicator, {Rotation = 90}, options.tween_time)
                        
                                    boxTween.Completed:Connect(function()
                                        objects.CanvasPosition = scrollPos
                                    end)
                                else -- Close
                                    dropdown_open = false
